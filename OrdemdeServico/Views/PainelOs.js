...
const PainelAcompanhamento = ({ navigation }) => {
  ...
  const fetchContadores = async () => {
    try {
      const response = await apiGetComContextoos('ordemdeservico/ordens/contadores/')
      console.log('📊 FRONTEND - Contadores do backend:', response)
      setContadores({
      ...
      const fallback = calcularContadores(ordens)
      setContadores(fallback)
    }
  }
  ...
  const fetchOrdens = async (filtros = {}, isLoadMore = false) => {
    if (isLoadMore) {
      ...
      setLoading(true)
      setPagination({ offset: 0, limit: 50, hasMore: true })
    }

    try {
      const params = new URLSearchParams()
      params.append('limit', pagination.limit.toString())
      params.append('offset', isLoadMore ? pagination.offset.toString() : '0')
      ...
      const url = `ordemdeservico/ordens/?${queryString}`
      ...
      if (Array.isArray(response)) {
        ...
        if (possibleArrays.length > 0) {
          ...
          totalCount = response.count || possibleArrays[0].length
        }
      }

      console.log('📊 FRONTEND - Total de registros:', ordensData.length, 'Total count:', totalCount)
      ...
      const newOffset = (isLoadMore ? pagination.offset : 0) + ordensData.length
            ...
            renderItem={({ item }) => (
              <View
                key={`${item.orde_empr || 'emp'}-${item.orde_fili || 'fil'}-${
                  item.orde_nume || 'num'
                }-${item.cliente_codigo || 'cli'}`}>
                {renderTableRow({ item })}
              </View>
            )}
            keyExtractor={(item, index) =>
              `${item.orde_empr || 'emp'}-${item.orde_fili || 'fil'}-${
                item.orde_nume || 'num'
              }-${index}`
            }
