...
def _alter_table(alias: str):
    with connections[alias].cursor() as cursor:
        # Usa bigint para armazenar referência a enti_clie com segurança
        cursor.execute("""
            ALTER TABLE entidades
            ADD COLUMN IF NOT EXISTS enti_arqu bigint;
        """)
...
class Command(BaseCommand):
    help = "Cria coluna enti_arqu em entidades para todos os tenants (ou um específico)."
    ...
    def handle(self, *args, **options):
        ...
        for lic in licencas:
            ...
            self.stdout.write(self.style.WARNING(f"[{alias}] Atualizando tabela entidades (adição de enti_arqu)..."))
            try:
                _alter_table(alias)
                self.stdout.write(self.style.SUCCESS(f"[{alias}] Campo enti_arqu atualizado/criado com sucesso!"))
            except Exception as e:
                self.stdout.write(self.style.ERROR(f"[{alias}] Erro ao atualizar campo enti_arqu: {e}"))
